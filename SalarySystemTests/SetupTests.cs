...
namespace SalarySystem.Tests
{
    ...
    {
        [TestMethod()]
        public void CreateAdminAccountTest_ShouldPass()
        {
        ...
        [TestMethod()]
        public void CreateAdminAccountTest_ShouldFail()
        {
            var admin = Setup.CreateAdminAccount();
            Admin adminTest = new Admin("admin", "admin123", 2500, "CEO");
            ...
            var expectedPassword = adminTest.Password;
            var actualPassword = admin.Password;
            Assert.AreNotEqual(expectedPassword, actualPassword);

            var expectedSalary = adminTest.Salary;
            var actualSalary = admin.Salary;
            Assert.AreNotEqual(expectedSalary, actualSalary);

            var expectedRole = adminTest.CompanyRole;
            var actualRole = admin.CompanyRole;
            Assert.AreNotEqual(expectedRole, actualRole);
        }
    }
}